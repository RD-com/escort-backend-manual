...
const approveGallery = async (req, res) => {
  try {
    ...
    const updateResult = await ApprovalStatus.update(
      { is_gallery_verified: isApproved },
    ...
    const successMsg =
      isApproved === "1"
        ? "Escort gallery approved Successfully!"
        : "Escort gallery rejected Successfully!";

...
const countProfileView = async (req, res) => {
  try {
    ...
    try {
      ...
      const result = await EscortStatics.increment("profile_view_count", {
        by: 1,
      ...
      res.status(200).json({
        success: true,
        msg: "Profile view count incremented successfully",
      });
    } catch (error) {
      console.error("Error processing the data:", error);
      res.status(400).json({ success: false, msg: "Profile view count error" });
    }
  } catch (error) {
    console.error("Error processing the data:", error);
    res.status(500).json({ success: false, msg: "Profile view count error" });
  }
};

module.exports = { countProfileView };
